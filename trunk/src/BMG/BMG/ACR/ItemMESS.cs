...
namespace BMG.ACR
{
    public class ItemMESS : BMG
    {
        public const string FileTitle = "ITEM.MESS";

        public override string FileType
        {
            get { return "Another Code R - ITEM.MESS"; }
        }
        ...
        public ItemMESS(Sentence_Item[] ss)
        {
            Sentences = ss;
        }

        public ItemMESS(string filename, bool rawBmg)
        {
            if (rawBmg)
            {
        ...
        private void ReadXmlFile(string filename)
        {
            ...
            Sentences = new Sentence_Item[sentenceNodes.Count];

            int i = 0;
            foreach (XmlElement element in sentenceNodes)
            {
                Sentences[i++] = new Sentence_Item(element);
            }
        ...
        private void ReadBMGFile(string filename)
        {
            ...
            using (FileStream stream = new FileStream(filename, FileMode.Open))
            {
                ...
                Sentences = new Sentence_Item[count];
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    Sentence_Item sen = new Sentence_Item(binData);
        ...
        public override void WriteSessionXml(string filename)
        {
            ...
            int i = 0;
            foreach (Sentence_Item sentence in Sentences)
            {
